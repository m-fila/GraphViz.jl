...
function init_cairo_structs!()
    ...
    julia_cairo_device[] = (
        gvplugin_installed_t(Int32(0), julia_cairo_name, Int32(0), unsafe_convert(Ptr{gvdevice_engine_t}, julia_cairo_engine), unsafe_convert(Ptr{gvdevice_features_t}, julia_cairo_features)),
...
function add_julia_cairo!(c::Context)
    lib = Ref{gvplugin_library_t}(gvplugin_library_t(
        julia_cairo_libname, unsafe_convert(Ptr{gvplugin_api_t}, julia_cairo_api)))
